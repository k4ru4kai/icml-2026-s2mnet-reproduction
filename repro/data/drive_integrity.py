...
class IntegrityResult:
    ...
    @property
    def ok(self) -> bool:
        """Return whether every structural rule passed."""

        return not self.errors and len(self.records) == 40
...
FILE_RULES = (
    ...
    FileRule(
        split="test",
        role="vessel annotation",
        relative_directory=Path("test/1st_manual"),
        filename_pattern=re.compile(r"(?P<id>\d{2})_manual1\.gif"),
        expected_ids=TEST_IDS,
    ),
    FileRule(
...
def _build_record(
    ...
    file_format = (
        f"image:{image_metadata.file_format}|"
        f"vessel_mask:{vessel_metadata.file_format}|"
...
def validate_drive(
    ...
    for split, expected_ids in (
        ("test", TEST_IDS),
        ("training", OFFICIAL_TRAINING_IDS),
    ):
        images = scanned[(split, "image")]
        vessels = scanned[(split, "vessel annotation")]
        fov_masks = scanned[(split, "FOV mask")]
        for image_id in expected_ids:
            if not all(image_id in mapping for mapping in (images, vessels, fov_masks)):
                errors.append(
                    f"Incomplete image/vessel/FOV correspondence for {split} ID {image_id}"
                )
                continue
            try:
                records.append(
                    _build_record(
                        ...
                        vessel_path=vessels[image_id],
                        fov_path=fov_masks[image_id],
    ...
    if len(records) != 40:
        errors.append(f"Expected 40 complete records, found {len(records)}")
